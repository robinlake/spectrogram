...
function setParentDimensions(config: CanvasConfig, parentElement: HTMLElement) {
  parentElement.style.height = Math.max(
    window.innerHeight * config.height,
    parentElement.clientHeight
  ).toString();
}

function createSpectrogramCanvas(
  config: CanvasConfig,
  ...
  return createCanvas(
    config,
    parentElement,
    drawSpectrogramCanvasFrame,
    throttled(drawColumns, 250)
  );
}
...
function createLegendCanvas(
  ...
  return createCanvas(
    config,
    parentElement,
    drawLegendCanvasFrame,
    throttled(drawLegend, 1000)
  );
}

function createOscilloscopeCanvas(
  config: CanvasConfig,
  parentElement: HTMLElement
): Canvas | null {
  return createCanvas(
    config,
    parentElement,
    drawOscilloscopeCanvasFrame,
    drawOscilloscopeVisual
  );
}

function createFrequencyCanvas(
  config: CanvasConfig,
  parentElement: HTMLElement
): Canvas | null {
  return createCanvas(
...
function drawColumns(canvas: Canvas, timeSeries: SpectralTimeSeries) {
  const decibelValuesToDisplay = timeSeries.getDecibelValuesForFrequencyRange(
    canvas.config.minDisplayedFrequency,
    canvas.config.maxDisplayedFrequency,
    timeSeries
  );
  canvas.context.fillStyle = "hsl(255, 100%, 50%)";
  const columnWidth =
    canvas.canvasElement.width / decibelValuesToDisplay.length;
  const binHeight =
    canvas.canvasElement.height /
    decibelValuesToDisplay[decibelValuesToDisplay.length - 1].length;
...
function drawOscilloscopeVisual(
  canvas: Canvas,
...
function drawBars(canvas: Canvas, timeSeries: SpectralTimeSeries) {
  ...
  const frequencies = timeSeries.getDisplayedFrequencies(
    canvas.config.minDisplayedFrequency,
    canvas.config.maxDisplayedFrequency,
    timeSeries
  );
  ...
  const mostRecentDecibelValues =
    decibelValuesToDisplay[decibelValuesToDisplay.length - 1];
  ...
  mostRecentDecibelValues.forEach((item, index) => {
    ...
    const localMax = isLocalMaximum(
      item,
      mostRecentDecibelValues.slice(
        Math.max(0, index - 3),
        Math.min(index + 3, mostRecentDecibelValues.length - 1)
      )
    );
...
const throttled = (callback: Function, interval: number) => {
  ...
  throttledFunc.changeInterval = (newInterval: number) =>
...
function drawLegend(canvas: Canvas, timeSeries: SpectralTimeSeries) {
  ...
  const displayedFrequencies = timeSeries.getDisplayedFrequencies(
  ...
  const iterationHeight =
    canvas.canvasElement.height / displayedFrequencies.length;
  ...
  for (const [index, frequency] of displayedFrequencies.entries()) {
    ...
    if (currentRowHeight >= minRowHeight) {
      ...
      canvas.context.strokeText(
        Math.round(frequency).toString() + " hz",
        0,
        canvas.canvasElement.height - height
      );
      currentRowHeight = 0;
    }
  }
...
function drawLegendCanvasFrame(canvas: Canvas, timeSeries: SpectralTimeSeries) {
  canvas.animationFrame = requestAnimationFrame(() =>
    drawLegendCanvasFrame(canvas, timeSeries)
  );
  canvas.animationFunction(canvas, timeSeries);
}
function drawSpectrogramCanvasFrame(
  canvas: Canvas,
  timeSeries: SpectralTimeSeries
) {
  canvas.animationFrame = requestAnimationFrame(() =>
    drawSpectrogramCanvasFrame(canvas, timeSeries)
  );
  timeSeries.pushDecibelValues(
    timeSeries.decibelValues,
    timeSeries.analyserNode,
    timeSeries.maxSampleCount
  );
  canvas.animationFunction(canvas, timeSeries);
}

function drawOscilloscopeCanvasFrame(
  canvas: Canvas,
  timeSeries: SpectralTimeSeries
) {
  canvas.animationFrame = requestAnimationFrame(() =>
    drawOscilloscopeCanvasFrame(canvas, timeSeries)
  );
  timeSeries.pushTimeDomainValues(
    timeSeries.timeDomainValues,
    timeSeries.analyserNode,
    timeSeries.maxSampleCount
  );
...
function drawFrequencyCanvasFrame(
  canvas: Canvas,
  timeSeries: SpectralTimeSeries
) {
  canvas.animationFrame = requestAnimationFrame(() =>
    drawFrequencyCanvasFrame(canvas, timeSeries)
  );
  timeSeries.pushDecibelValues(
    timeSeries.decibelValues,
    timeSeries.analyserNode,
